...
BUILTIN_DATASETS = {
    ...
    'ml-1m':
        BuiltinDataset(
            ...
            path=join(get_dataset_dir(), 'ml-1m/ml-1m/ratings.csv'),
            item_path=join(get_dataset_dir(), 'ml-1m/ml-1m/movies.csv'),
            reader_params=dict(line_format='user item rating timestamp',
                               rating_scale=(1, 5),
                               sep='::')
        ),
    'ml-latest-small':
        BuiltinDataset(
            ...
            item_path=join(get_dataset_dir(), 'ml-latest-small/movies.csv'),
            reader_params=dict(line_format='user item rating timestamp',
                               rating_scale=(1, 5),
                               sep=',')
        ),
